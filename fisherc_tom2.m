...
function W = fisherc_tom2(varargin)
% should work for all scenario's (regularized and not)
% and uses correct y's.

  mapname = 'Fisher Regularized Tom 2';
  ...
  if mapping_task(argin,'definition')
    ...
    %W = setname(W,'Fisher Regularized');
    W = define_mapping(argin,'untrained',mapname);
    return;
  end
  
  [a,lambda] = deal(argin{:});
  %fprintf('lambda is %g',lambda);
  ...
  if numel(L) < 2                  % demand at least 2 classes
    ...
    %W = onec(a);
    error('only found 1 class. stopping!');
    return;
  end
  ...
  if islabtype(a,'crisp') && c > 2 % multi-class classification
    W = mclassc(a,fisherc_tom2([],lambda));        % combine one-against-rest classifiers
  ...
  if islabtype(a,'crisp')
      % this is already done. dont change +1/-1
  else
    %y = invsigm(y); % better to give [0,1] targets full range
    error('dont do that!'); % we only have crisp labels!
  ...
  if (lambda > 0)
      v = inv(b'*b + lambda*eye(dim))\(b'*y);
      % this also regularizes the bias term which is not so nice.
      % but ok, thats too bad
  else
